...
% 18273-pro, 18275-ser, 18360-tyr]
bux_ban_aa_of_interest = bux;
% bux_ban_aa_of_interest(18273) = 0; % (*******)
...
indices_targeted_metabolite = find(strcmp(ecModel1.metNames, 'L-Proline')); % (*******)
row_targeted_metabolite = sum(S_rm_prot_pool(indices_targeted_metabolite,:), 1);
rxns_pro_engaged = find(row_targeted_metabolite ~= 0)';
targeted_rxn = (rxns_pro_engaged == 2617); % (*******)
rxns_pro_engaged_that_remain_unchanged_flux = rxns_pro_engaged(~targeted_rxn);
blx_case = blx_add_biomass_lb;
blx_case(rxns_pro_engaged_that_remain_unchanged_flux) = v_ref(rxns_pro_engaged_that_remain_unchanged_flux); 
bux_case = bux_ban_aa_of_interest;
bux_case(rxns_pro_engaged_that_remain_unchanged_flux) = v_ref(rxns_pro_engaged_that_remain_unchanged_flux); 
% case状态下，pro参与的反应的flux (除2617外) 与ref状态保持一致
