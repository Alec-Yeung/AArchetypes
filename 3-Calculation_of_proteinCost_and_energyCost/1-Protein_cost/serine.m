...
% 18273-pro, 18275-ser, 18360-tyr]
bux_ban_aa_of_interest = bux;
% bux_ban_aa_of_interest(18275) = 0; % (*******)
...
% [552-ala, 658-arg, 553-asn, 149-asp, 555-cys, 559-gln,123-glu, 561-gly, 
% 2493-pro, 563-ser, 2091-tyr]
S_rm_prot_pool_ds(563, biomass_id) = S_rm_prot_pool_ds(563, biomass_id) + ds; % (*******)

indices_targeted_metabolite = find(strcmp(ecModel1.metNames, 'L-Serine')); 
row_targeted_metabolite = sum(S_rm_prot_pool(indices_targeted_metabolite,:), 1);
rxns_ser_engaged = find(row_targeted_metabolite ~= 0)';
targeted_rxn = (rxns_ser_engaged == 8960); 
rxns_ser_engaged_that_remain_unchanged_flux = rxns_ser_engaged(~targeted_rxn);
blx_case = blx_add_biomass_lb;
blx_case(rxns_ser_engaged_that_remain_unchanged_flux) = v_ref(rxns_ser_engaged_that_remain_unchanged_flux); 
bux_case = bux_ban_aa_of_interest;
bux_case(rxns_ser_engaged_that_remain_unchanged_flux) = v_ref(rxns_ser_engaged_that_remain_unchanged_flux); 
% case状态下，ser参与的反应的flux (除8960外) 与ref状态保持一致
