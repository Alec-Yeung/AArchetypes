...
rxns_tyr_engaged = find(row_targeted_metabolite ~= 0)';
targeted_rxn = (rxns_tyr_engaged == 2689); % (*******)
rxns_tyr_engaged_that_remain_unchanged_flux = rxns_tyr_engaged(~targeted_rxn);
blx_case = blx_add_biomass_lb;
blx_case(rxns_tyr_engaged_that_remain_unchanged_flux) = v_ref(rxns_tyr_engaged_that_remain_unchanged_flux); 
bux_case = bux_ban_aa_of_interest;
bux_case(rxns_tyr_engaged_that_remain_unchanged_flux) = v_ref(rxns_tyr_engaged_that_remain_unchanged_flux); 
% case状态下，tyr参与的反应的flux (除2689外) 与ref状态保持一致
