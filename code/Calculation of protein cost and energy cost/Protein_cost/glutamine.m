...
% 18273-pro, 18275-ser, 18360-tyr]
bux_ban_aa_of_interest = bux;
% bux_ban_aa_of_interest(18358) = 0; % (*******)
...
% [552-ala, 658-arg, 553-asn, 149-asp, 555-cys, 559-gln,123-glu, 561-gly, 
% 2493-pro, 563-ser, 2091-tyr]
S_rm_prot_pool_ds(559, biomass_id) = S_rm_prot_pool_ds(559, biomass_id) + ds; % (*******)

indices_targeted_metabolite = find(strcmp(ecModel1.metNames, 'L-Glutamine')); % (*******)
row_targeted_metabolite = sum(S_rm_prot_pool(indices_targeted_metabolite,:), 1);
rxns_gln_engaged = find(row_targeted_metabolite ~= 0)';
targeted_rxn = (rxns_gln_engaged == 8900); % (*******)
rxns_gln_engaged_that_remain_unchanged_flux = rxns_gln_engaged(~targeted_rxn);
blx_case = blx_add_biomass_lb;
blx_case(rxns_gln_engaged_that_remain_unchanged_flux) = v_ref(rxns_gln_engaged_that_remain_unchanged_flux); 
bux_case = bux_ban_aa_of_interest;
bux_case(rxns_gln_engaged_that_remain_unchanged_flux) = v_ref(rxns_gln_engaged_that_remain_unchanged_flux); 
% case状态下，gln参与的反应的flux (除8900外) 与ref状态保持一致

[r_case, res_case] = mosekopt('minimize echo(2)', struct('c', c_protein_cost, 'a', ...
    S_rm_prot_pool_ds, 'blc', blc, 'buc', buc, 'blx', blx_case, 'bux',...
    bux_case));
