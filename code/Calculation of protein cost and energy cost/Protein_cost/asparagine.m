...
% 18273-pro, 18275-ser, 18360-tyr]
bux_ban_aa_of_interest = bux;
% bux_ban_aa_of_interest(18235) = 0; % (*******)
...
% [552-ala, 658-arg, 553-asn, 149-asp, 555-cys, 559-gln,123-glu, 561-gly, 
% 2493-pro, 563-ser, 2091-tyr]
S_rm_prot_pool_ds(553, biomass_id) = S_rm_prot_pool_ds(553, biomass_id) + ds; % (*******)

indices_targeted_metabolite = find(strcmp(ecModel1.metNames, 'L-Asparagine')); % (*******)
row_targeted_metabolite = sum(S_rm_prot_pool(indices_targeted_metabolite,:), 1);
rxns_asn_engaged = find(row_targeted_metabolite ~= 0)';
targeted_rxn = (rxns_asn_engaged == 456); % (*******)
rxns_asn_engaged_that_remain_unchanged_flux = rxns_asn_engaged(~targeted_rxn);
blx_case = blx_add_biomass_lb;
blx_case(rxns_asn_engaged_that_remain_unchanged_flux) = v_ref(rxns_asn_engaged_that_remain_unchanged_flux); 
bux_case = bux_ban_aa_of_interest;
bux_case(rxns_asn_engaged_that_remain_unchanged_flux) = v_ref(rxns_asn_engaged_that_remain_unchanged_flux); 
% case状态下，asn参与的反应的flux (除456外) 与ref状态保持一致
